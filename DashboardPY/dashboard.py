...
print("Depois de ler o arquivo CSV")  # Adicione esta linha para debug


# Convertendo mes para data e único
...
fig_prod = px.bar(df_filtered, x="Date"
                             , y="Product line"
                             , color="City"
...
fig_kind = px.pie(df_filtered, values="Total"
                             , names="Payment"
                             , title="Faturamento por Tipo de Pagamento")
col4.plotly_chart(fig_kind, use_container_width=True)

city_total = df_filtered.groupby("City")[["Rating"]].mean().reset_index()
fig_rating = px.bar(df_filtered, x="City"
                             , y="Rating"
                             , title="Avaliação")
